...
def run_inference(cfg_path, input_path, model_path, output_path):
    ...
    save_path = os.path.join(
        output_path, os.path.basename(input_path).replace(".npz", "_pred.npz")
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--weights", required=True, help="Path to pretrained SONATA model"
    )
    parser.add_argument(
        "--output", default="output", help="Directory to save predictions"
    )
