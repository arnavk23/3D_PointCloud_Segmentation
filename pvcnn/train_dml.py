...
def prepare():
    ...
    if configs.evaluate is None:
        configs.train.deep_mutual_learning = configs.train.get(
            "deep_mutual_learning", True
        )
        metrics = []
        ...
        configs.train.best_checkpoint_paths = {
            m: os.path.join(
                best_checkpoints_dir, "best.{}.pth.tar".format(m.replace("/", "."))
            )
            for m in configs.train.metrics
        }
        ...
        if configs.train.deep_mutual_learning:
            configs.train.best_student_checkpoint_path = os.path.join(
                save_path, "best_student.pth.tar"
            )
...
def main():
    ...
    def train(
        ...
        for inputs, targets in tqdm(loader, desc="train", ncols=0):
            ...
            if model_student is None:
                ...
                loss = criterion(outputs, targets) + criterion_dml(
                    outputs_student, outputs
                )
                loss_student = criterion(outputs_student, targets) + criterion_dml(
                    outputs, outputs_student
                )
                writer.add_scalar("loss/train", loss.item(), current_step)
                writer.add_scalar(
                    "loss/train_student", loss_student.item(), current_step
                )
                current_step += targets.size(0)
                loss.backward()
                optimizer.step()
    ...
    if os.path.exists(configs.train.checkpoint_path):
        ...
        if configs.train.deep_mutual_learning:
            if (
                "model_student" in checkpoint
                and checkpoint["model_student"] is not None
            ):
                print(" => loading model_student")
                model_student.load_state_dict(checkpoint.pop("model_student"))
            if (
                "optimizer_student" in checkpoint
                and checkpoint["optimizer_student"] is not None
            ):
            ...
            for m in configs.train.metrics:
                best_metrics_student[m] = meters_student.get(
                    m + "_best", best_metrics_student[m]
                )
    ...
    if "scheduler" in configs.train and configs.train.scheduler is not None:
        ...
        if "T_max" in configs.train.scheduler:
            ...
            print(
                f"==> modify scheduler T_max to {configs.train.scheduler.T_max} {configs.train.scheduler_unit}s"
            )
        print(
            f'==> creating scheduler "{configs.train.scheduler}" from {last_unit} {configs.train.scheduler_unit}'
        )
    ...
    if last_epoch >= configs.train.num_epochs:
        ...
        for split, loader in loaders.items():
            if split != "train":
                ...
                if configs.train.deep_mutual_learning:
                    meters_student.update(
                        evaluate(model_student, loader=loader, split=split)
                    )
        for k, meter in meters.items():
            print(f"[{k}] = {meter:2f}")
    ...
    with tensorboardX.SummaryWriter(configs.train.save_path) as writer:
        for current_epoch in range(last_epoch + 1, configs.train.num_epochs):
            ...
            train(
                ...
                writer=writer,
                schedule_per_epoch=schedule_per_epoch,
                model_student=model_student,
                criterion_dml=criterion_dml,
            ...
            best, best_student = {m: False for m in configs.train.metrics}, {
                m: False for m in configs.train.metrics
            }
            for m in configs.train.metrics:
                ...
                if configs.train.deep_mutual_learning:
                    if (
                        best_metrics_student[m] is None
                        or best_metrics_student[m] < meters_student[m]
                    ):
                        best_metrics_student[m], best_student[m] = (
                            meters_student[m],
                            True,
                        )
            ...
            for m in configs.train.metrics:
                ...
                if best_student[m]:
                    shutil.copyfile(
                        configs.train.checkpoint_path,
                        configs.train.best_student_checkpoint_paths[m],
                    )
            if best.get(configs.train.metric, False):
                shutil.copyfile(
                    configs.train.checkpoint_path, configs.train.best_checkpoint_path
                )
            if best_student.get(configs.train.metric, False):
                shutil.copyfile(
                    configs.train.checkpoint_path,
                    configs.train.best_student_checkpoint_path,
                )
