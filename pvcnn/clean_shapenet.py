...
def clean_json_file(filepath):
    ...
    cleaned = [
        x for x in data if "/" in x and x.count("/") == 1 and not x.startswith("Bag/")
    ]
...
with open(
    "data/shapenet_part/train_test_split/shuffled_train_file_list_clean.json", "r"
) as f:
    data = json.load(f)
...
with open(
    "data/shapenet_part/train_test_split/shuffled_train_file_list_clean.json", "w"
) as f:
