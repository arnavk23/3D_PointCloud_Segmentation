...
class PointCloudSegmenter:
    ...
    def cluster_points(self, points, features=None):
        ...
        n_clusters = min(
            50, len(np.unique(filtered_labels))
        ...
        for cluster_id in np.unique(gmm_labels):
            ...
            xy_span = np.ptp(
                cluster_pts[:, :2], axis=0
            )  # Fixed: using np.ptp() instead of .ptp()
    ...
    def visualize(self, points, labels):
        ...
        vis.create_window(
            window_name="Point Cloud Segmentation", width=1024, height=768
        )
    ...
    def save_segmented_ply(self, points, labels, output_path):
        ...
        colors = (self.color_map(labels / (self.num_classes - 1))[:, :3] * 255).astype(
            np.uint8
        )
    ...
    def analyze_results(self, labels):
        ...
        for cls, cnt in zip(unique, counts):
            print(
                f"{self.class_names[cls]:<15}: {cnt:>6} points ({cnt/len(labels):.1%})"
            )
